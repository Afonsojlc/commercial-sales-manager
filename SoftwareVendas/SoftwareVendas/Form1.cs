...
namespace SoftwareVendas
{
    public partial class Form1 : Form
    {
        ...
        #region Adi��o de Produtos � Tabela

        private void button1_Click(object? sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtCodigoProduto.Text))
            {
                MessageBox.Show("Por favor, introduza o c�digo do produto.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            using (SqlConnection con = DatabaseConfig.ObterConexao())
            {
                try
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        ...
                        using (SqlDataReader leitor = cmd.ExecuteReader())
                        {
                            ...
                            {
                                MessageBox.Show("Produto n�o encontrado na base de dados.", "N�o encontrado", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
                ...
                {
                    MessageBox.Show($"Ocorreu um erro de comunica��o com a base de dados.\nDetalhes: {ex.Message}", "Erro de Conex�o", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private void AtualizarTotais()
        {
            ...
            decimal totalIVA = totalIliquido * 0.23m; // Assumindo taxa fixa de 23% para simplifica��o global
        ...
        #region Finalizar Venda (Transa��o SQL)

        private void btnFinalizar_Click(object? sender, EventArgs e)
        {
            if (dgvItens.Rows.Count == 0)
            {
                MessageBox.Show("N�o existem artigos na lista para finalizar a venda.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrWhiteSpace(txtNIF.Text))
            {
                MessageBox.Show("� necess�rio identificar o cliente atrav�s do NIF.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            // --- VERIFICA��O PROFISSIONAL ANTES DE ABRIR A BASE DE DADOS ---
            if (modoModificacao)
            {
                DialogResult respostaConfirmacao = MessageBox.Show(
                    $"Est� prestes a modificar a Encomenda N� {idEncomendaEditar}.\n\nO novo valor total ser� de {valorTotalVenda.ToString("C2")}.\nO invent�rio (stock) ser� reajustado automaticamente.\n\nConfirma estas altera��es?",
                    "Confirmar Altera��o",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Warning,
                    MessageBoxDefaultButton.Button2); // Foca no 'N�o' por seguran�a para evitar cliques acidentais
            ...
            using (SqlConnection con = DatabaseConfig.ObterConexao())
            {
                ...
                try
                {
                    ...
                    using (SqlCommand cmdBusca = new SqlCommand(queryBuscaID, con, transacao))
                    {
                        ...
                        {
                            transacao.Rollback();
                            MessageBox.Show("Cliente n�o encontrado na base de dados. Por favor, verifique o NIF inserido.", "Erro de Valida��o", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            return;
                        }
                    }
                    ...
                    foreach (DataGridViewRow row in dgvItens.Rows)
                    {
                        ...
                        // Obter Taxa de IVA espec�fica do produto
                    ...
                    if (modoModificacao)
                    {
                        MessageBox.Show($"As altera��es na encomenda N� {numEncomenda} foram guardadas e o invent�rio foi atualizado com sucesso.", "Altera��o Conclu�da", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.DialogResult = DialogResult.OK; // D� sinal verde � janela anterior
                        this.Close(); // Fecha a janela de edi��o
                    }
                    else
                    {
                        MessageBox.Show($"Venda n.� {numEncomenda} registada e invent�rio atualizado com sucesso.", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        LimparVenda();
                    }
                }
                catch (Exception ex)
                {
                    transacao.Rollback();
                    MessageBox.Show($"N�o foi poss�vel concluir a transa��o.\nTodas as altera��es foram revertidas.\nDetalhe: {ex.Message}", "Erro Cr�tico", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        #region Autocomplete e Gest�o de Clientes
        ...
        private void CarregarAutoCompletar()
        {
            ...
            using (SqlConnection con = DatabaseConfig.ObterConexao())
            {
                ...
                catch { /* Continua a execu��o sem autocompletar em caso de falha de liga��o n�o cr�tica */ }
            }
        ...
        private void dgvItens_CellEndEdit(object? sender, DataGridViewCellEventArgs e)
        {
            ...
            // Atualiza��o de Produto mediante altera��o manual de C�digo
            if (e.ColumnIndex == 0)
            {
                ...
                using (SqlConnection con = DatabaseConfig.ObterConexao())
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, con))
                        {
                            ...
                            using (SqlDataReader leitor = cmd.ExecuteReader())
                            {
                                ...
                                {
                                    MessageBox.Show("O c�digo inserido n�o existe no invent�rio.", "C�digo Inv�lido", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    ...
                    {
                        MessageBox.Show($"Ocorreu um erro ao validar o c�digo da grelha: {ex.Message}", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }

            // Recalcular totais perante altera��es na linha
        ...
        private void dgvItens_KeyDown(object? sender, KeyEventArgs e)
        {
            ...
            {
                DialogResult resposta = MessageBox.Show("Confirma a remo��o dos artigos selecionados?", "Remover Artigo", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                if (resposta == DialogResult.Yes)
                {
                    // Evitar modifica��o da cole��o durante a itera��o
        ...
        private void btnRemoverLinha_Click(object? sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Selecione uma linha v�lida para remover.", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        // --- INTEGRA��O COM FORMUL�RIO DE PRODUTOS ---
        ...
        public void PrepararModoModificacao(int idEncomenda)
        {
            ...
            this.Text = "Modificar Encomenda N� " + idEncomenda;

            // Procura o bot�o de finalizar e muda o texto e cor
            ...
            if (btnControls.Length > 0 && btnControls[0] is Button btnFinalizar)
            {
                ...
                btnFinalizar.BackColor = Color.FromArgb(243, 156, 18); // Laranja para alertar que � edi��o
            }
        ...
        private void CarregarDadosParaEdicao(int id)
        {
            using (SqlConnection con = DatabaseConfig.ObterConexao())
            {
                ...
                {
                    MessageBox.Show("Erro ao carregar dados para edi��o: " + ex.Message);
                }
            }
        }

        // Handles n�o utilizados deixados vazios por depend�ncia do designer
